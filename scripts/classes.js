class flower{
    ...
    for(var i=0;i<this.numLeafs;i++){
            ...
            rotate(this.yPos*.01+radians(i*360/this.numLeafs));
    ...
    move() {
        ...
        this.xPos = this.xPos + 2*sin(this.yPos/random(30,50))+ (mouseX-width/2)*.1;
        if(this.xPos < 0 - 2*this.rad) this.xPos = windowWidth * 1/this.scalef;
        if(this.xPos >  windowWidth * 1/this.scalef) this.xPos = 0;
        if(this.yPos < 0 - 2*this.rad ) this.yPos = windowHeight * 1/this.scalef;
        lastmousex = mouseX;
      }
...
class textreveal{
    ...
    show(){
        if (this.reveal < this.sourceText.length) this.reveal += .5;
        text(this.sourceText.substring(0, this.reveal), this.x, this.y);
    }
    unshow(){
        if (this.reveal > 0) this.reveal -= .5;
        text(this.sourceText.substring(0, this.reveal), this.x, this.y);
    }
}
/*
